...
export default function NotificationList({ initialNotifications }) {
  ...
  // The screen this is most needed on had no live updates at all — every
  // other place a notification's existence matters (the sidebar badge in
  // ChatList.js, the unread count in app/(main)/layout.js) already
  // subscribes to this table, but the actual list a visitor is looking at
  // right now didn't, so a notification arriving while this was open just
  // sat missing until a manual reload.
  //
  // userId is looked up here rather than threaded down as a prop: the
  // server component above already scopes getNotifications() by the
  // authenticated user internally, so there was previously no reason for
  // this id to exist on the client at all.
  useEffect(() => {
    ...
    async function subscribe() {
      ...
      channel = supabase
        ...
        .on('postgres_changes', {
          ...
          // Guards against a duplicate if this INSERT's response race
          // ever lands after a manual refresh already fetched the same
          // row — de-duped by id rather than assumed impossible.
        ...
        .on('postgres_changes', {
          ...
          // Covers this same notification being marked read from another
          // device/tab — markNotificationRead already updates local state
          // directly for the device the tap happened on, this is only for
