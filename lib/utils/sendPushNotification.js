...
export async function sendPushNotification(userId, title, body, url, conversationId, type) {
  try {
    ...
    let isHiddenConversation = false
    ...
    if (conversationId) {
      ...
      // Hiding a conversation is a privacy action, but the push for it was
      // still going out with the sender's name and a message preview — so a
      // chat someone had deliberately tucked away announced itself on their
      // lock screen, in front of exactly the person they were hiding it
      // from. That made hiding close to pointless in the one situation it
      // exists for.
      //
      // Redacted rather than suppressed: dropping the push entirely would
      // silently swallow messages, which is its own bug. The recipient
      // still learns something arrived, just not what or from whom, and
      // the deep link goes to the hidden shelf instead of naming the
    ...
    const payload = JSON.stringify({
      title: isHiddenConversation ? 'New message' : title,
      body: isHiddenConversation ? 'Open Relay to view' : body,
      url: isHiddenConversation ? '/chat/hidden' : (url || '/'),
