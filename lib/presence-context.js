...
export function PresenceProvider({ userId, children }) {
  ...
  useEffect(() => {
    ...
    channel
      ...
      // currentPresences is what's still tracked under this key after the
      // leave — non-empty when the same user has another tab/device still
      // connected (each tracks under the same presence key: userId).
      // Removing the key unconditionally meant closing/backgrounding one
      // tab showed that user as offline everywhere else in the app even
      // while they were still online in another tab, until the next
      // unrelated presence change happened to trigger a full 'sync'.
